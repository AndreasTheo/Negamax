...
Board
---------------------------
Board Class for a Connect4/N Game
*/
public class Board implements IBoard {
    ...
    private int rowScoreToWin = 3;
    private ZobristHashTable hTable;
    ...
    */
	...
	hasWon = false;
	hTable = new ZobristHashTable(rows * columns, numOfCoins);
	hTable.createUniqueKeys();
    }
    ...
    */
	...
	if (position >= 0 && position < columns) {
		...
		if (board[i][currPos.getyPos()] == 0) {
		    ...
		    currPos.setxPos(i);
		    int moveHashKey = hTable.getHashTable()[(i * columns) + currPos.getyPos()][coinNumber-1];

		    if (boardStateHistory.size() > 0) {
			int prevMoveHashKey = boardStateHistory.peek().getUniqueBoardKeyAtLastMove();
			int uniqueBoardKeyAtMove = moveHashKey ^ prevMoveHashKey;
			boardStateHistory.push(new BoardState(currPos, hasWon, uniqueBoardKeyAtMove));
		    } else {
			boardStateHistory.push(new BoardState(currPos, hasWon, moveHashKey));
		    }
		    break;
		}
	    }
...
*/
  ...
  */
    ...
    /*
	Get Board Configuration Key
	---------------------------
	This method returns the configuration key for the current board using the history stack of moves where the last move
	holds a unique key associated with the current board configuration.
*/
    @Override
    public int getBoardConfigurationKey() {
	if (boardStateHistory.size() > 0) {
	    return boardStateHistory.peek().getUniqueBoardKeyAtLastMove();
	}
	return 0;
    }
	...
	this.rowScoreToWin = rowScoreToWin;
    }
